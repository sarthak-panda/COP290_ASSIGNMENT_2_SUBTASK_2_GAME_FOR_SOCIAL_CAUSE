...
function removeMission(missionName) {
    ...
    for (var i = 0; i < missions.length; i++) {
        if (missions[i].innerText === missionName) {
            ...
            var completedMissions = 7 - missionList.getElementsByTagName('li').length;
            document.querySelector('#mission-button span').textContent = completedMissions;

            points += 100; // Assuming each mission is worth 100 points
            updatePoints();
            break;
        }
    }
...
function updatePoints() {
    ...
    pointsElement.innerHTML = `<img src="https://upload.wikimedia.org/wikipedia/commons/e/e8/Rotating-golden-star.gif" alt="Star" />  ${points}`;
}


// Example usage: Remove mission named "MISSION_3"

// removeMission("MISSION_4");
setTimeout(function() {
    removeMission("MISSION_3");
}, 7000);
...
function Run() {
  ...
  var runAnimation = requestAnimationFrame(Run);
